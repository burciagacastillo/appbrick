...
export function FormaGasto({ propiedadId }: { propiedadId: string }) {
  ...
  return (
    <form action={agregarGasto} className="grid gap-2 px-4 py-3 sm:grid-cols-6">
      <input type="hidden" name="propiedadId" value={propiedadId} />

      <label className="text-xs sm:col-span-1">
        <span className="text-slate-500">Fecha</span>
        <input type="date" name="fecha" defaultValue={hoy} className={inputCls} />
      </label>
      ...
      <label className="text-xs sm:col-span-1">
        ...
        <select name="categoria" defaultValue="material" className={inputCls}>
          {GRUPOS_GASTO.map((g) => (
      ...
      <label className="text-xs sm:col-span-1">
        ...
        <select name="metodo" defaultValue="transferencia" className={inputCls}>
          {METODOS_PAGO.map((m) => (
      ...
      <label className="text-xs sm:col-span-2">
        <span className="text-slate-500">Quién pagó</span>
        <input
          name="pagadoPor"
          placeholder="Erick, Aragón, Enrique…"
          className={inputCls}
        />
      </label>
